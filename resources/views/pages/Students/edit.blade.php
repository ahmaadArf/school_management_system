        ...
        <div class="card card-statistics h-100">
            <div class="card-body">
                ...
                @endif
                    ...
                    <form action="{{route('dashboard.students.update',$Students->id)}}" method="post" autocomplete="off">
                        @method('PUT')
                        @csrf
    ...
    <script>
        $(document).ready(function () {
            $('select[name="Grade_id"]').on('change', function () {
                ...
                if (Grade_id) {
                    $.ajax({
                        url: "{{ URL::to('dashboard/Get_classrooms') }}/" + Grade_id,
                        type: "GET",
                        dataType: "json",
    ...
    <script>
        $(document).ready(function () {
            $('select[name="Classroom_id"]').on('change', function () {
                ...
                if (Classroom_id) {
                    $.ajax({
                        url: "{{ URL::to('dashboard/Get_Sections') }}/" + Classroom_id,
                        type: "GET",
                        dataType: "json",
                        success: function (data) {
